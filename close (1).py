...
def wait_motion_started(bus, timeout_s=0.5):
    # Tras el ACK, el control task todavia no ha escrito RUNNING en 0x0C:
    # alli puede quedar el estado anterior (IDLE=0 o SUCCEEDED=2). Sin esta
    # espera, wait_motion_finished saldria de inmediato con un valor obsoleto.
    deadline = time.monotonic() + timeout_s
...
def main():
    ...
    with SMBus(BUS_NUM) as bus:
        ...
        if result != 1:
            raise RuntimeError(f"CLOSE rechazado: {RESULT_NAMES.get(result, result)}")

        print("CLOSE aceptado. Esperando fin del movimiento...")
        final_state = wait_motion_finished(bus)

        print(f"CLOSE terminado: {final_state} ({MOTION_NAMES.get(final_state, 'UNKNOWN')})")
